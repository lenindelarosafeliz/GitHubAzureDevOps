var config = {
    grid: {
                ...
                if (target == 'w2ui-details') {
                    data.onComplete = function () {
                        ...
                        w2ui['detail'].add([
                            ...
                            { recid: 2, name: 'PersonaId', value: record.personaId  },
                            { recid: 3, name: 'DocumentoTipo', value: record.documentoTipo  },
                            { recid: 4, name: 'Número', value: record.numero  }
                        ]);
        ...
        searches: [
            ...
            { field: 'personaId', text: 'PersonaId', type: 'int'},
            { field: 'documentoTipoId', text: 'DocumentoTipo', type: 'int' },
            { field: 'numero', text: 'Número', type: 'text'}
        ],
        sortData: [{ field: 'description', direction: 'DESC' }],
        columns: [
            { field: 'id', text: 'Id', sortable: true, resizable: true },
            { field: 'personaId', text: 'PersonaId', sortable: true, resizable: true },
            { field: 'documentoTipoId', text: 'DocumentoTipo', sortable: true, resizable: true },
            { field: 'numero', text: 'Número', sortable: true, resizable: true }
        ],
...
// init
$(function () {
    $('#documentoGrid').w2grid(config.grid);
...
function deleteRecord() {
    ...
    if (selection.length) {
        ...
        if (selectedId.id > 0) {
            $.ajax({
                type: 'POST',
                url: '/Documentos/Delete/' + selectedId.id,
                async: true
            }).done(function (e) {
